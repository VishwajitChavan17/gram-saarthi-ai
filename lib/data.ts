...
export const getSchemesByStatus = (status: 'ongoing' | 'upcoming'): Scheme[] => {
...
export const getUpcomingSchemes = (): Scheme[] => {
  return mockSchemes.filter(scheme => scheme.status === 'upcoming');
};

export const getOngoingSchemes = (): Scheme[] => {
  return mockSchemes.filter(scheme => scheme.status === 'ongoing');
};

export const getAllCategories = (): string[] => {
  return Array.from(new Set(mockSchemes.map(scheme => scheme.category)));
};
